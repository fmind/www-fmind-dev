...
- New private drafts enter `content/articles/` through `pub export` from `~/fmind/publications` on `article/*` branches. Once `posts/published.md` records the live site URL, the private `article.md` is removed and this repository owns the only published body. Later corrections use the `article-manual-edit` PR label; a substantial generated revision must start from the current site Markdown.
